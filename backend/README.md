...
Copy `backend/.env.example` to `backend/.env` for local development and pass it to manual Compose commands with `docker compose --env-file backend/.env ...`. The example `DATABASE_URL` matches the Docker Compose `postgres` service documented in [../docs/LOCAL_DATABASE.md](../docs/LOCAL_DATABASE.md): database `mediqaz`, user `superuser`, password `superpassword`, host port `54329`.

The example `TEST_DATABASE_URL` matches the Docker Compose `postgres_test` service: database `mediqaz_test`, user `superuser`, password `superpassword`, manual host port `54330`. Automated runners may replace the port with a repository-derived value so parallel checkouts do not collide.
...
Auth and authenticated account-management writes use `AUTH_BODY_LIMIT_BYTES` and `AUTH_RATE_LIMIT_*` bounded in-process fixed-window limiters. A conflicting cross-account claim for an existing Expo token quarantines delivery without transferring installation ownership; the authorized installation can re-enable the token by registering with its secret. `TRUST_PROXY=false` uses the direct Bun connection address. Behind a trusted proxy, set `TRUST_PROXY=true` together with the provider's authoritative `TRUSTED_PROXY_CLIENT_IP_HEADER`; use `TRUSTED_PROXY_CLIENT_IP_POSITION=last` when the provider appends the client to a comma-separated chain. On Railway that is the last `X-Forwarded-For` value. The default shape is one API instance. Before horizontally scaling, move rate-limit state to a shared trusted store or edge/WAF layer.
...
- Cron: `bun run start:cron -- <task>`, backed by `src/cron.ts`. Available tasks are `noop`, `db:ping`, `notifications:process`, `auth:sessions:cleanup`, and the recommended combined `maintenance:process`.
...
Production deployment for the backend uses Railway with Railway managed PostgreSQL, built from `backend/Dockerfile` and configured by `railway.json` at the repository root. Migrations run in the pre-deploy command. Follow the runbook in [../docs/DEPLOYMENT.md](../docs/DEPLOYMENT.md) instead of duplicating provider steps here.
...
The storage service lives in `src/storage` and wraps an S3-compatible object store through the S3 SDK. It stays unconfigured until the product needs uploads. Product-specific upload routes should validate ownership and permissions, then delegate object key generation, presigned upload/download URLs, public CDN URL construction, and deletion to that service.
